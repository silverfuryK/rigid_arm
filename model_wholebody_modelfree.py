...
class CassieArmEnv(Env):
	...
	def __init__(
		...
		arm_policy_model_path="arm_best_model",
	...
	def get_random_data(self, play = False, random = True):
		...
		if random:
			self.data_obs[6] = np.random.uniform(-2, 2)
			self.data_obs[7] = np.random.uniform(-2, 2)
			self.data_obs[8] = np.random.uniform(0.01, 1.2)
			self.data_ee_target = self.data_obs[6:]
	...
	def step(self, action):
		...
		action_arm_mixed = current_arm_corrections

		self.arm_corrections = current_arm_corrections
		
		# action_arm_mixed = mixing_coeff*np.clip(action_arm_og, a_min = -arm_bounds, a_max = arm_bounds)
		...
		sigmas = [0.5, 1, 1, 1, 1, 0.4, 0.8, 0.5]
		reward_weights = [0.8, 0.4, 0.7, 0.1, 0.0, 10, 0.1, 0.9]
		...
		print("ref_ee_position", ref_end_effector_position,"current_ee_position",current_endeffector_position, "\n\n\n\n\n")
		
		self.del_ee_pos = (np.linalg.norm(current_endeffector_position - ref_end_effector_position))
		...
		R = (
			...
			+ 0.2 * self.reward_cassie_pelvis_acc
			+ 0.1 * np.exp(-(np.linalg.norm(action, ord=2) ** 2))
			+ 1
			+ foot_reward
			# + 0.1*np.exp(-(np.linalg.norm(action, ord=2) ** 2))
	...
	def calculate_foot_reward(self, weight_in_polygon = 2, weight_foot_ori = 0.2, weight_heel_toe_z = 1.5, weight_foot_y = 0.5):
