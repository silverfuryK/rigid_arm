...
if __name__ == '__main__':
    ...
    wandbcallback = WandbCallback(
        model_save_path=f"models/PPO{run.id}",
    ...
    eval_callback = EvalCallback(
        eval_env,
        best_model_save_path=f"./logs/PPO{run.name}/",
        log_path=f"./logs/PPO{run.name}/",
        eval_freq=2000,
        n_eval_episodes=10,
        deterministic=True,
        render=False,
    ...
    model = PPO(
        ...
        tensorboard_log=f"logs/tensorboard/PPO{run.name}/",
        policy_kwargs=dict(
    ...
    model.save("./models_no_action_trackPPO")