...
class ArmEnv(Env):
	...
	def _get_obs(self):
		...
		self.end_effector_ori, self.end_effector_pos = end_effector_ori, end_effector_pos
	...
	def step(self, action, target_pose = None, write_to_file = False):
		...
		current_arm_corrections = self.arm_corrections + mixing_coeff*np.clip(action, a_min = -arm_bounds, a_max = arm_bounds)
		action_c = current_arm_corrections
		self.arm_corrections = action_c

		action_vel = np.array([0,0,0,0,0,0])

		actions = np.concatenate((action, action_vel))
		...
		ee_target_ori, ee_target_pos = self.compute_relative_pose(self.data_obs[9:], self.data_obs[6:9])
		...
		actual_ee_ori, actual_ee_pos = self.compute_relative_pose(self.end_effector_ori, self.end_effector_pos)

		target_action = self.data_action[0:6]
		actual_action = actions[0:6]


		self.r1 = np.exp(-np.linalg.norm(ee_target_pos - actual_ee_pos))
		...
		reward =    (
					...
					0.0 * np.exp(-np.linalg.norm(action))
		)
	...
	def reset(self, target_pose = None, random = False, seed = None, play = False):
		target_pose = None
		obs, info = self.reset_model(target_pose = target_pose, random_sample=random, play = play)
		return obs, info
