...
class ArmEnv(Env):
	...
	def get_random_data(self, play = False):
	...
	def step(self, action, target_pose = None, write_to_file = False):
		...
		actual_ee_pos = observation[12:15]
		actual_ee_ori = observation[15:19]
		...
		reward =    reward_weights[0]*self.r1 + \
		...
		self.del_ee_pos = np.abs(np.linalg.norm(ee_target_pos)-np.linalg.norm(actual_ee_pos))

		if self.del_ee_pos < 0.01:
				print("del_ee_pos: ", self.del_ee_pos)
				print("pose reached within 1cm")
	...
	def reset_model(self, target_pose = None):
		...
		if target_pose is None:
			self.get_random_data(play = self.play)
		else:
			self.data_obs = target_pose
