...
class CassieArmEnv(Env):
	...
	def __init__(
		self,
		xml_string='xml/cassie_w_soft_arm.xml',
		...
		arm_policy_model_path="softarm_IK",
	...
	def step(self, action):
		...
		""" get acitons from the policy and apply it to the mujoco model for the arm 
			...
			cassie position
		"""
		action_arm = self.arm_action_pred
		...
		action_arm_mixed = action_arm + mixing_coeff*np.clip(action_arm_og, a_min = -clipping, a_max = clipping)

		actions_vel = np.zeros(6)

		action = np.concatenate((action_cassie, action_arm_mixed)) 
		...
		current_endeffector_position = self.data.sensor("end-effector-pos").data
		# swap z and x columns
		# current_endeffector_position = current_endeffector_position[[2, 1, 0]]
		current_endeffector_orientation = self.data.sensor("end-effector-orientation").data
		...
		sigmas = [1, 1, 1, 1, 1, 0.4, 0.8, 0.5]
		reward_weights = [0.4, 0.4, 0.7, 0.1, 0.0, 1, 0.1, 0.9]
		...
		self.del_ee_pos = np.linalg.norm(current_endeffector_position - ref_end_effector_position, ord = 2)
		...
		self.reward_cassie_qpos = -(np.linalg.norm(ref_qpos[3:7] - current_pelvis_ori, ord=2) ** 2) / (1* 1)
		...
		R = (
			...
			# + reward_weights[7] * self.reward_cassie_qpos
			+ 0.9 * self.reward_z
			+ 0.9 * self.reward_cassie_qpos
			+ 0.5 * self.reward_cassie_pelvis_vel
			+ 0.001*np.exp(-(np.linalg.norm(action, ord=2) ** 2))
			+ 1
			+ foot_reward
	...
	def calculate_foot_reward(self, weight_in_polygon = 2, weight_foot_ori = 1, weight_heel_toe_z = 1):
		...
		reward_foot_ori = weight_foot_ori * np.exp(
		...
		self.reward_foot_ori = np.exp(
			-(np.linalg.norm(left_foot_ori - right_foot_ori, ord=2) / (3 * 1 ** 2))
		)
	...
	def reset_model(self, target_pose = None, seed = None):
		...
		if target_pose is not None:
			self.data_obs[0:3] = target_pose


		self.arm_action_pred = self.get_arm_action_predicition(self.data_obs[0:3], self.data_obs[3:7])
	...
	def reset(self, target_pose = None, seed = None):
		target_pose = None
		obs, info = self.reset_model(target_pose = target_pose)
		return obs, info
